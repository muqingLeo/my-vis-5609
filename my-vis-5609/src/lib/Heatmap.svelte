<script lang="ts">
    ...
    let { movies, width = 800, height = 700 }: Props = $props()
    ...
    const colorScale = $derived(() =>
        d3.scaleSequential(d3.interpolateCool).domain([0, maxCount()])
    )
...
{#if movies.length > 0}
    <svg {width} {height}>
            ...
            {#each Object.entries(pairCounts()) as [pair, count]}
                {#if count > 0}
                    {#each pair.split('-') as genre1, i}
                        {#if i === 0}
                            <!-- Added ARIA so the style lint won't keep compaining
             I consulted ChatGPT on why "a11y warnings in Svelte" are happening. -->
                            <rect
        ...
        <g transform="translate({width - 100}, {height / 4})">
            <text x="0" y="-10" font-size="14" font-weight="bold"
                >Co-occurrence Scale</text
            >
