<script lang="ts">
    interface MovingMesh extends THREE.Mesh {
        speed: number;
    }
    ...
    const morphs: Array<MovingMesh> = []; // Array to store the morphs (animated models: horse, flamingo, parrot)
    ...
    onMount(
        ...
        async () => {
            ...
            console.log("CSV loading result:", rawData);
            ...
            // Initialise scene after data is loaded.
            init(window.innerWidth, window.innerHeight, genreCounts);
        },
    );

    function init(SCREEN_WIDTH: number, SCREEN_HEIGHT: number, genreCounts: TGenreCount) {
        // Renderer
        ...
        fontLoader.load("3d/helvetiker_bold.typeface.json", (font: Font) => {
            ...
            createBars(scene, font, genreCounts);
        });
    ...
    function createBars(scene: THREE.Scene, font: Font, genreCounts: TGenreCount) {
        console.log("genreCounts: Loaded:", genreCounts);
        ...
        const yScale = d3
            .scaleLinear()
            .domain([0, Math.max(...Object.values(genreCounts).map((d) => d))])
            .range([0, maxHeight]);

        // Create bars for each genre
        Object.keys(genreCounts).forEach((genre, i) => {
            console.log("Adding bar:", genre, genreCounts[genre]);

            const bar = createOneBar(
                yScale(genreCounts[genre]),
                xScale.bandwidth(),
            );
            bar.position.set(
                xScale(genre)!,
                FLOOR + yScale(genreCounts[genre]) / 2,
                0,
            );
            scene.add(bar);
            ...
            addLabelToBar(
                ...
                xScale(genre)!,
                FLOOR + yScale(genreCounts[genre]) + 20,
                bar.position.z + 10,
                font,
            );
        });
    ...
    function addLabelToBar(
        ...
        // textMesh.rotation.x = -Math.PI / 5; // Rotate the text to be horizontal
        // textMesh.rotation.y = 0; // Rotate the text to be horizontal
        textMesh.rotation.set(0, 0, Math.PI / 2); 
        // textMesh.rotation.set(0, -Math.PI / 4, 0); // ~45°
